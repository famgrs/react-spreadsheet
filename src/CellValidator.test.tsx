...
const MOCK_VALIDATOR = jest.fn((it) => it % 2 === 0);
const EXAMPLE_INVALID_DATA: Types.CellBaseValidator = {
  value: 1,
...
describe("<CellValidator />", () => {
  test("renders", () => {
    render(<CellValidator {...EXAMPLE_PROPS} />);
    const element = document.querySelector(".Spreadsheet__cell");
    expect(element).not.toBeNull();
    expect(element).not.toHaveClass("Spreadsheet__cell--invalid");
  ...
  test("renders invalid", () => {
    render(<CellValidator {...EXAMPLE_PROPS} data={EXAMPLE_INVALID_DATA} />);
    const element = document.querySelector(
      ".Spreadsheet__cell.Spreadsheet__cell--invalid"
    );
    expect(element).not.toBeNull();
    expect(MOCK_VALIDATOR).toBeCalledTimes(1);
    expect(MOCK_VALIDATOR).toBeCalledWith(EXAMPLE_INVALID_DATA.value);
    expect(MOCK_DATA_VIEWER).toBeCalledTimes(1);
    expect(MOCK_DATA_VIEWER).toBeCalledWith(
      { ...EXAMPLE_DATA_VIEWER_PROPS, cell: EXAMPLE_INVALID_DATA },
      {}
    );
    expect(MOCK_SET_CELL_DIMENSIONS).toBeCalledTimes(0);
  });
  test("renders read only", () => {
    render(<CellValidator {...EXAMPLE_PROPS} data={EXAMPLE_READ_ONLY_DATA} />);
    const element = document.querySelector(
  ...
  test("renders with given class name", () => {
    render(
      <CellValidator {...EXAMPLE_PROPS} data={EXAMPLE_DATA_WITH_CLASS_NAME} />
    );
    const element = document.querySelector(
  ...
  test("renders selected", () => {
    render(<CellValidator {...EXAMPLE_PROPS} selected />);
  ...
  test("renders active", () => {
    render(<CellValidator {...EXAMPLE_PROPS} active />);
    const element = document.querySelector<HTMLElement>(".Spreadsheet__cell");
    expect(element).not.toBeNull();
    if (!element) {
      throw new Error("element must be defined");
  ...
  test("handles mouse over with dragging", () => {
    render(<CellValidator {...EXAMPLE_PROPS} dragging />);
    const element = document.querySelector<HTMLElement>(".Spreadsheet__cell");
    expect(element).not.toBeNull();
    if (!element) {
      throw new Error("element must be defined");
  ...
  test("custom cell DataViewer", () => {
    render(
      <CellValidator
        {...EXAMPLE_PROPS}
        data={EXAMPLE_DATA_WITH_CUSTOM_DATA_VIEWER}
      />
    );
    const element = document.querySelector(".Spreadsheet__cell");
    expect(element).not.toBeNull();
